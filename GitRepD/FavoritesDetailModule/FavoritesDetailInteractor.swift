            ...
            if let result = try? context.fetch(request) {
                if (result != []) {
                    for result in result {
                        self.singleRepository = SingleRepository(id: Int(result.id), name: result.name!, created_at: result.dateCreated, language: result.languageUsed, description: result.repoDescription, url: result.url)
                    }
